...
% Author: Jesse Sharp; Last Update: 13/03/2019
...
U = zeros(1,N); %Initial guess for the chemotherapy and immunotherapy controls
...
%Initialisation of the state. Here we use the approximate steady state of the system for the given parameters
S(1) = 1-gs/ps; 
A(1) = 0.3255; 
X(1) = 0.5207; 
L(1) = 0.3715; 
...
Costate = @(t,Lambda,y,U) [-(-2*y(1)*Lambda(1)*ps-gs*Lambda(1)+gs*Lambda(2)+Lambda(1)*ps);
    ...
    ux*Lambda(3);
    -(a2-pa*y(2)*Lambda(2)-Lambda(4)*pl*y(2)-2*pl*y(4)*Lambda(4)+Lambda(4)*pl-Lambda(4)*gl-Lambda(4)*Alpha/(Gamma+y(4))+Lambda(4)*Alpha*y(4)/(Gamma+y(4))^2-U*Lambda(4)+gl*Lambda(5));
    ut*Lambda(5)];
...
Uupdate = max(Uupper*((sign(-y(4,:).*Lambda(4,:)+a1)-1)/(-2)),Ulower*((sign(-y(4,:).*Lambda(4,:)+a1)+1)/(2))); %Calculated updated control control
U = omega*U + (1-omega)*Uupdate; %Actual updated control after applying relaxation to aid convergence

% %To view the solution as it converges, uncomment this block (line 115 to line 130)   to produce interim
% %figures
% box on
