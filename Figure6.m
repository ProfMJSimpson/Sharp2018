...
Uupdate = max(upper*((sign(-y(4,:).*Lambda(4,:)+a1)-1)/(-2)),lower*((sign(-y(4,:).*Lambda(4,:)+a1)+1)/(2))); %Calculated updated control control
U = omega*U + (1-omega)*Uupdate; %Actual updated control after applying relaxation to aid convergence

% %To view the solution as it converges, uncomment this block (line 112 to line 127)   to produce interim
% %figures
% box on
